class VisionSystem {
    ...
    draw(ctx, enemyX, enemyY, polygonPoints) {
        ...
        // Tüm noktaları sırayla çiz
        for (let i = 0; i < polygonPoints.length; i++) {
            ctx.lineTo(polygonPoints[i].x, polygonPoints[i].y);
        }
        
        // EKSİK OLAN KOD BURASI: Dış çemberi tamamlamak için 
        // son noktadan sonra tekrar İLK noktaya bir çizgi çek.
        ctx.lineTo(polygonPoints[0].x, polygonPoints[0].y);
        
        ctx.closePath(); // Şimdi merkeze güvenle dönüp poligonu kapatabiliriz
        ctx.fill();
    }
}